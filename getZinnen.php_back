...
function updateStatusForThema($conn){
    ...
    if (!empty($thema) && !empty($status)) {
        ...
        $sql = "UPDATE sentences SET status = ? WHERE thema = ?";
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            echo json_encode(['success' => false, 'error' => 'Prepare failed: ' . $conn->error]);
            exit();
...
function saveStatus($conn){
    $rowId = $_POST['rowId'] ?? '';
    $status = $_POST['status'] ?? '';

    logAction($conn, 'saveStatus', ['rowId' => $rowId, 'status' => $status]);

    if (!empty($rowId) && !empty($status)) {
        // SQL query to update the sentence in the database
        $sql = "UPDATE sentences SET status = ? WHERE ID = ?";
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            echo json_encode(['success' => false, 'error' => 'Prepare failed: ' . $conn->error]);
            exit();
...
function saveStatusAnnotatie($conn){
    ...
    if (!empty($rowId) && !empty($statusAnnotatie)) {
...
function saveStatusGlos($conn){
    ...
    if (!empty($rowId) && !empty($statusGlos)) {
...
function saveStatusVideo($conn){
    ...
    if (!empty($rowId) && !empty($statusVideo)) {
...
function fetchVideos($conn){
    ...
    if (!empty($rowId)) {
        ...
        $sql = "SELECT * FROM matched_transcriptions WHERE m_transcription = ? AND zOg = 'Zin'";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $rowId);
        if (!$stmt->execute()) {
...
function fetchSentences($conn) {
    ...
    if (!empty($sentenceIDs)) {
        ...
        while ($video_row = $video_result->fetch_assoc()) {
            ...
            $srt_gebaar_voor_gebaar = __DIR__ . '/eaf/zin/' . $video_basename . '_Gebaar_voor_gebaar.srt';
...
SUM(CASE WHEN COALESCE(status, '0') = 'Klaar'
       AND COALESCE(status_annotatie, '0') = 'Klaar'
       AND COALESCE(status_glos, '0') = 'Klaar'
       AND COALESCE(status_video, '0') = 'Klaar'
       THEN 1 ELSE 0 END) AS ready_sentences,
SUM(CASE WHEN COALESCE(status, '0') = 'Klaar' THEN 1 ELSE 0 END) AS count_status_klaar,
...
SUM(CASE WHEN COALESCE(status, '0') <> 'Klaar' THEN 1 ELSE 0 END) AS count_status_not_klaar,
...
SUM(CASE WHEN COALESCE(status_glos, '0') <> 'Klaar' THEN 1 ELSE 0 END) AS count_status_glos_not_klaar
    ...
    echo json_encode(array_merge(['success' => true, 'eaf_files' => $eafCount], $counts));
    exit();
}
...
function fetchZinArrayByFilename($conn) {
    ...
    $sql_s = "SELECT zinArray FROM sentences WHERE ID = ? LIMIT 1";
    ...
    // Return the zinArray
    echo json_encode(['success' => true, 'zinArray' => $zinArray]);
    exit();
}
